...
// este guard es para que una ves que se halla logueado osea ya halla una identity, no pueda escribir ni login ni register en la url (este guard lo pusimos a esas rutas)
export class NoIdentityGuard implements CanActivate{
    ...
    canActivate(){
        ...
        if (identity && identity.name) {//validamos que halla la identity
            this._router.navigate(['/']);//si  hay, que nos rediriga a la ruta que sea / home
